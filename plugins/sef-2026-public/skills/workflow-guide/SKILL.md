---
name: workflow-guide
description: 공공 프로젝트 전체 생성 워크플로우 가이드. Phase 1(기획) → Phase 2(설계) → Phase 3(초기화) → Phase 4(개발) → Phase 5(품질검토) → Phase 6(배포) 순서로 어떤 스킬/에이전트를 사용할지 안내. "공공 프로젝트 만들어줘", "어디서부터 시작해야 해", "전체 순서 알려줘" 요청 시 사용.
---
...
```
Phase 1: 기획           Phase 2: 설계            Phase 3: 초기화
prd-generator       →   tech-stack           →   project-init (공공)
prd-validator           architecture-validator    frontend-init
development-planner     db-schema-designer
                        api-designer
                        screen-spec-writer
                                 ↓
Phase 4: 개발 (도메인별 반복)
  Backend:  sef-2026-public:backend
  Frontend: module-generator → page-generator → component-builder (선택)
                                 ↓
Phase 5: 품질 검토          Phase 6: 배포
code-reviewer           →   sef-2026-public:deployment
security-auditor
test-generator
reference-updater
```
...
**스킬**: `sef-2026-public:project-init`
...
# 참조: deployment/references/jeus-deployment.md
# 참조: deployment/references/weblogic-deployment.md
...
□ 9.  project-init (공공)   백엔드 프로젝트 구조 생성
□ 10. frontend-init         프론트엔드 초기화

  [도메인별 반복]
□ 11. backend               백엔드 모듈 개발
□ 12. module-generator      프론트 types + composable 생성
□ 13. page-generator        프론트 페이지 생성
□ 14. component-builder     복잡 컴포넌트 생성 (선택)

  [검토]
□ 15. code-reviewer         코드 리뷰
□ 16. security-auditor      보안 검토
□ 17. test-generator        테스트 생성
□ 18. reference-updater     문서 일치 검증

  [배포]
□ 19. deployment (공공)     WAR 빌드 → WAS 배포
```
...
- `sef-2026-public:project-init`: 백엔드 초기 구조 생성
- `sef-2026-public:backend`: 백엔드 모듈 개발
- `sef-2026-public:deployment`: WAR 빌드 및 배포
